    ...
    internal abstract class QuestionBase
    {
        ...
        public static QuestionBase[] CreateBaseQuestions(int size)
        {
            ...
            for (int i = 0; i < questions?.Length; i++)
            {
                ...
                do
                {
                    Console.WriteLine($" Type of the Question Number {i + 1} (1- T/F Question, 2-  Choose one Quesion, 3- for MCQ ) ");

                } while (!int.TryParse(Console.ReadLine(), out questionType) || questionType < 1 || questionType > 3);

                if (questionType == 1)
                {
                    Console.WriteLine($" True or False Question Number {i + 1}");
                    questions[i] = TFQuestions.AddTFQuestion();
                }
                else if (questionType == 2)
                {
                    Console.WriteLine($" Choose One Question Number {i + 1}");
                    questions[i] = ChooseOneQuestions.AddChooseOneQuestion();

                }
                else if (questionType == 3)
                {
                    Console.WriteLine($" MCQ Question Number {i + 1}");
